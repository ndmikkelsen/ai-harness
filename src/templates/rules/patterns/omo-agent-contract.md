...
2. Repo-local plans, workflow docs, and verification artifacts are planning/execution truth.
...
| `bd ready/update/close` with repo-local workflow docs | Beads + local execution workflow | `bd` unavailable or `.beads/` missing | continue with documented repo-local workflow and record missing tracker state in handoff | `grep -n "continue with repo-local workflow guidance" .rules/patterns/beads-integration.md` |
