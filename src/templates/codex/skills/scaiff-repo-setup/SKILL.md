...
3. Run `scaiff` with `--init-json` so you can distinguish `createdPaths` from `skippedPaths`
4. In existing repos, customize only the files listed in `createdPaths`, guided by `references/scaffold-customization-map.md`
5. Run `scaiff doctor <target> --assistant <codex|opencode>` after setup
6. Summarize what was created, what was preserved, and any follow-up gaps
