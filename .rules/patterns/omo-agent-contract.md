...
2. Repo-local plan docs, workflow artifacts, and verification evidence are planning/execution truth.
3. Cognee is required broadly by lane policy in this contract.
4. Worktree bootstrap follows deterministic seam ordering.

## Lane Rules
- `planning`: read `.rules/`, local plan context (including `.planning/STATE.md` when present), and active issue context first; produce plans and decision records, not code changes.
- `research`: gather repo and Cognee context, summarize with file references, and hand findings to planning or execution.
- `execution`: implement within assigned file boundaries, keep Beads and verification evidence aligned, and produce verification-ready changes.
...
| --- | --- | --- | --- | --- | --- |
| `prometheus` | planning | `.rules/`, local plan context, active issue context, Cognee brief | read/search tools, planning docs, repo mapping | direct landing, closing issues, creating competing planning systems | `sisyphus`, `hephaestus` |
...
| `sisyphus` | execution | canonical contract, active plan context, claimed Beads issue, Cognee brief | full repo tools needed for scoped implementation and verification | bypassing Beads state or verification evidence, direct promotion to `main`, undocumented policy invention | `momus`, `oracle`, `atlas` |
| `hephaestus` | execution | assigned task context, `.rules/`, local plan context, relevant tests | file edits, tests, build/typecheck, scoped implementation tools | planning-system changes outside task scope, landing without authority | `sisyphus`, `momus` |
...
| `explore` | research | contract, relevant module paths, `.rules/`, local plan context | repo search/read tools, structural mapping | mutations, landing, issue closure | `prometheus`, `sisyphus` |
...
| --- | --- | --- | --- | --- |
| planning | `.rules/`, local plan context, active issue context, Cognee brief | plan docs, read/search tools, decision tables | code mutation without delegated execution scope, landing, issue closure | execution |
...
| execution | contract, assigned task, local plan context, claimed issue, relevant tests | file edits, tests, build/typecheck, deterministic verification | closing issues before verification, direct promotion to `main`, creating policy drift | review |
...
| `bd ready/update/close` with repo-local workflow docs | Beads + local execution workflow | `bd` unavailable or `.beads/` missing | continue with documented repo-local workflow and record missing tracker state in handoff | `grep -n "continue with repo-local workflow guidance" .rules/patterns/operator-workflow.md .rules/patterns/beads-integration.md` |
| `./.codex/scripts/cognee-brief.sh` | planning/research lanes | Cognee bridge unavailable | follow lane policy outcome and use local `.rules/` plus current repo plans/docs | `grep -n "Cognee" .rules/patterns/operator-workflow.md .rules/patterns/omo-agent-contract.md` |
...
- `execution` lanes consume the latest Cognee brief when present and may continue with local `.rules/` plus current repo plans/docs if the contract explicitly marks the task as fallback-safe.
