...
for file in "$REPO_ROOT/.planning/PROJECT.md" "$REPO_ROOT/.planning/REQUIREMENTS.md" "$REPO_ROOT/.planning/STATE.md" "$REPO_ROOT/.planning/ROADMAP.md"; do
  [[ -f "$file" ]] && "$BRIDGE" upload "$file" --dataset "$DATASET"
done

while IFS= read -r -d '' file; do
  "$BRIDGE" upload "$file" --dataset "$DATASET"
done < <(find "$REPO_ROOT/.planning/phases" \( -name 'PLAN.md' -o -name 'SUMMARY.md' -o -name 'RESEARCH.md' -o -name 'VERIFICATION.md' -o -name 'UAT.md' -o -name 'CONTEXT.md' \) -print0 2>/dev/null)
