...
describe('CLI install-skill', () => {
  it('installs the OpenCode skill bundle from the CLI', async () => {
    ...
    const result = await execFile(
      process.execPath,
      [tsxCli, 'src/cli.ts', 'install-skill', '--assistant', 'opencode', '--target-root', targetRoot, '--config-root', configRoot, '--json'],
      {
        cwd: repoRoot,
        encoding: 'utf8'
      }
  ...
  it('prints local launcher guidance in the human-readable report', async () => {
    ...
    const result = await execFile(
      process.execPath,
      [tsxCli, 'src/cli.ts', 'install-skill', '--assistant', 'opencode', '--target-root', targetRoot, '--config-root', configRoot],
      {
        cwd: repoRoot,
        encoding: 'utf8'
      }
