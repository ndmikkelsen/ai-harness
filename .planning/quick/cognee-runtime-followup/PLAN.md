...
- Live verification no longer stops at proxy `504`s: upload/cognify now surface Cognee's 30-second LLM connection-test `409`, and search surfaces the underlying OpenAI rate-limit failure from the current credential path.
...
2. Once `ai-harness-rl2` is landed, resolve the operator credential/provider blocker in `ai-harness-3rj` by using a funded LLM key or a supported alternate provider and rerunning upload/cognify/query checks against the committed deploy path.
3. After queryability is verified, complete `ai-harness-9nr` by capturing the operator contract in durable docs, including health, upload, cognify, and query checks plus required secret/provider expectations.

## Execution Order

1. `ai-harness-rl2` (in progress)
...
2. `ai-harness-3rj` (blocked by `ai-harness-rl2`)
...
3. `ai-harness-9nr` (blocked by `ai-harness-3rj`)
