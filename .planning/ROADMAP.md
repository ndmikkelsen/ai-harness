...
- Phase 3 is verified: `ai-harness` is now explicitly a local-use, non-registry tool, and downstream repos get scaffold baseline markers, preserve-by-default upgrade guidance, local install/update docs, and explicit `scaiff` migration messaging through `.planning/phases/phase-3-distribution-readiness/` and Beads epic `ai-harness-b42`.
